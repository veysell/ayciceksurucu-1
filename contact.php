<?php
$page = "İletişim | ";
include("inc/head.php")
?>

<!-- Page Header Start -->
<div class="container-fluid page-header py-5 mb-5">
    <div class="container py-5">
        <h1 class="display-3 text-white mb-3 animated slideInDown">İLETİŞİM</h1>
        <nav aria-label="breadcrumb animated slideInDown">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a class="text-white" href="#">ANASAYFA</a></li>
                <li class="breadcrumb-item text-white active" aria-current="page">İLETİŞİM</li>
            </ol>
        </nav>
    </div>
</div>
<!-- Page Header End -->


<!-- Contact Start -->
<div class="container-fluid bg-light overflow-hidden px-lg-0" style="margin: 6rem 0;">
    <div class="container contact px-lg-0">
        <div class="row g-0 mx-lg-0">
            <div class="col-lg-6 contact-text py-5 wow fadeIn" data-wow-delay="0.5s">
                <div class="p-lg-5 ps-lg-0">
                    <div class="section-title text-start">
                        <h1 class="display-5 mb-4">BİZE ULAŞIN </h1>
                    </div>
                    <div class="container">
                        <div class="row mb-4">
                            <div class="col-2"><a class="btn btn-primary btn-lg-square btn-social" href=""><i
                                        class="fab fa-instagram"></i></a></div>
                            <div class="col">
                                <h3>İnstagramdan Takip Edin</h3>
                            </div>
                        </div>
                        <div class="row mb-4">
                            <div class="col-2"><a class="btn btn-primary btn-lg-square btn-social" href="https://api.whatsapp.com/send?phone=905493197171"><i
                                        class="fab fa-whatsapp"></i></a></div>
                            <div class="col">
                                <h3>Whatsapp Üzerinden Ulaşın</h3>
                            </div>

                        </div>

                        <div class="row mb-4">
                            <div class="col-2"> <a class="btn btn-primary btn-lg-square btn-social" href=""><i
                                        class="fab fa-youtube"></i></a></div>
                            <div class="col">
                                <h3>Youtube Kanalımızı Ziyaret Edin</h3>
                            </div>
                        </div>
                        <div class="row mb-4">
                            <div class="col-2">
                                <a class="btn btn-primary btn-lg-square btn-social" href="tel:+905493197171"><i
                                        class="fa fa-phone-alt"></i></a>
                            </div>
                            <div class="col">
                                <h3>Bizi Direkt Arayın</h3>
                            </div>
                        </div>

                    </div>

                </div>
            </div>
            <div class="col-lg-6 pe-lg-0" style="min-height: 400px;">
                <div class="position-relative h-100">
                    <!-- <iframe class="position-absolute w-100 h-100" style="object-fit: cover;"
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3001156.4288297426!2d-78.01371936852176!3d42.72876761954724!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4ccc4bf0f123a5a9%3A0xddcfc6c1de189567!2sNew%20York%2C%20USA!5e0!3m2!1sen!2sbd!4v1603794290143!5m2!1sen!2sbd"
                        frameborder="0" allowfullscreen="" aria-hidden="false"
                        tabindex="0"></iframe> -->
                    <iframe class="position-absolute w-100 h-100" style="object-fit: cover;"
                        src="https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=Yeni%C5%9Fehir%20Mah.%20266%20sok%20No:12/A+(My%20Business%20Name)&amp;t=&amp;z=17&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"><a
                            href="https://www.maps.ie/population/">Population calculator map</a></iframe>
                </div>
            </div>
        </div>
    </div>
</div>
</div>
<!-- Contact End -->

<?php
include("inc/footer.php")
?>